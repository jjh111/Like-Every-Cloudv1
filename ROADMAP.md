...
- **2026-05-19** — Ambient + per-hero audio system + hero audio mixer + branding/UI cluster. **Audio**: encoded the first batch of SFX/ambient WAVs from `Working - gitignore/Sounds/` (fan / sewing / generator / motorcycle) to AAC-LC m4a via `npm run compress-audio`: **467.77 MB → 15.84 MB (-96.6%)**. Added 4 placeholder GLBs (procedural spheres, generated by `npm run make-placeholder-heroes`) for `hero_sewing`, `hero_fan`, `hero_motorcycle`, `hero_generator` — drop-in real geometry later without touching the manifest. Extended `states.json` schema with `heroAudio` map keyed by heroId — each sound-emitting hero is authored on/off per state, with per-state volume. `buildStateRules` synthesizes ambient-channel beds with spatial anchoring + 0.6s crossfade on state change. Old `ambient`/`ambientVolume` keys silently migrated to `music`/`musicVolume` for back-compat. Generator sources (SND62906/SND80837) appear to be commercial library — license check needed before public ship. **Mixer**: new dev-only `heroAudioMixer` panel (top-left, under the badge) with per-hero on/off toggle + per-hero volume slider + save-to-states.json button — toggles are bidirectional and inherit a template from the other state when flipping ON in a state where the hero isn't authored. New `/__lec/save-states` vite middleware + `saveStatesConfig` flow round-trip the full StateConfig with a hand-curated formatter. AudioManager got `setEntryVolume`, `getEntryVolume`, `isEntryPlaying` for the mixer to drive without re-instantiating sources. **Branding/UI**: shipped `public/like-every-cloud.svg` wordmark — large in the loader overlay (replacing the text title), small glass-card badge top-left of the experience (height-fitted so swapping square↔wordmark logos doesn't need code changes). Clustered the sound controls at bottom-center: `audioControls` (mute pill + master volume) now sits directly above the existing tracks bar instead of bottom-left.
- **2026-05-18** — Cloudflare Pages prep + fork-based hosting: added `.nvmrc` pinning Node 20 for the build environment; removed unused hero GLBs (`hero_amp`, `hero_cassette`, `hero_monoblock_chair`, `hero_speaker`) from `public/heroes/` so they don't ship to the host; stripped `.DS_Store` from `public/`. Created `jjh111/Like-Every-Cloud` fork to work around Cloudflare's OAuth-into-repo-owner constraint — Cloudflare watches the fork, `origin` remains the client's repo. `npm run deploy` does `git push origin main && git push fork main` in one command. **Total `dist/` is now ~31 MB** (was 36 MB), audio placeholders accounting for ~23 MB of that. First deploy attempt revealed Cloudflare's Vite preset wants Vite 6+ for its Workers+Vite auto-config — added `wrangler.jsonc` declaring `./dist` as static assets so wrangler skips the auto-config entirely. README's Hosting section documents the topology, fresh-clone setup, the `wrangler.jsonc` purpose, and client-handoff steps.
